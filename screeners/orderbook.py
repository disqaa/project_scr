...
def fetch_orderbook_walls(min_size_usdt: float, max_distance_pct: float, client, symbols_limit: int = 20):
    symbols = client.get_spot_symbols(symbols_limit)
    prices = client.get_spot_tickers()
    walls = {}
    for symbol in symbols:
        current_price = prices.get(symbol)
        if not current_price or current_price == 0:
            continue
        orderbook = client.get_orderbook(symbol, limit=150)
        if not orderbook:
            continue
